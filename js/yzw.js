function AjaxPolling(taskId, key, serverUrl, options) {
    this.taskId = taskId,
        ...
        this.useProgressBar = !1,
        this.onError = function(xhr, retryCount) {},
        this.onSuccess = function(data) {},
        this.everyPolling = function(data) {
            this.useProgressBar && this.changeProgressBar(data)
        },
        this.onTimeOut = function() {},
        this.pollingCount = 0,
        this.pollingLimit = options.pollingLimit || 10,
        this.pollingDelay = options.delay || 1000,
        this.delayGaps = options.delayGaps || 0,
        this.retryCount = 0,
        this.retryLimit = options.retryLimit || 10,
        this.retryAfter = options.retryAfter || 1000,
        this.retryGaps = options.retryGaps || 300
}

function return_html(flag, data, code, codeType) {
    return flag || (code = ''),
        template('common/tbtjzy-item', {
            seachList: data,
            code: code,
            agent_from: mark.agent_from,
            codeType: codeType
        })
}
...
function seach_ssdm(f) {
    var ss_code = $('#ssdm').val(),
        _html1 = template('common/qecx-seach-item', {
            seachList: AreaList[0].sub,
            selectitem: ss_code
        }),
        _html2 = template('common/qecx-seach-item', {
            seachList: AreaList[1].sub,
            selectitem: ss_code
        });
    $('#ssdm').html('<option value="">请选择' + ('wap' == mark.agent_from ? '省市' : '') + '</option><optgroup label="' + AreaList[0].name + '">' + _html1 + '</optgroup><optgroup label="' + AreaList[1].name + '">' + _html2 + '</optgroup>'),
        null != getQueryString('id') ? seach_dwdm(!0) : ($('#dwxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '单位' : '') + '</option>'), $('#yxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '院系所' : '') + '</option>'), $('#zyxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '专业' : '') + '</option>'), $('#fxxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '研究方向' : '') + '</option>'), $('#xxfs').html('<option value="">请选择' + ('wap' == mark.agent_from ? '学习方式' : '') + '</option>'), $('.tj-tsyq-warp').not('tj-hide').addClass('tj-hide'), $('#tsyq').text('招生单位对该专业暂无特殊要求'))
}

function seach_dwdm(f) {
    $('.tj-tsyq-warp').not('tj-hide').addClass('tj-hide'),
        $('#tsyq').text('招生单位对该专业暂无特殊要求');
    var ss_code = $('#ssdm').val(),
        dw_code = $('#dwxx').val();
    init_ajax(Mainfest['tbtjzy-dwxx'].interface_url + '?code=' + ss_code, function(o) {
        $('#dwxx').html(return_html(f, o, dw_code, '1')),
            f ? seach_yxxx(!0) : ($('.dwxx-web').removeClass('mar-b0'), $('.tb-tip-time').hide(), $('#adjust_id').val(''), $('#yxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '院系所' : '') + '</option>'), $('#zyxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '专业' : '') + '</option>'), $('#fxxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '研究方向' : '') + '</option>'), $('#xxfs').html('<option value="">请选择' + ('wap' == mark.agent_from ? '学习方式' : '') + '</option>'))
    })
}
...
function seach_yxxx(f) {
    ...
    var dw_code = $('#dwxx').val(),
        yx_code = $('#yxxx').val();
    ksbh.substring(0, 5) == dw_code || 1 == _sxbz || 1 != sxbz2 && 1 != sxbz3 ? ($('#js-sxbz').html(''), $('.submit-tr .submit-btn').attr('disabled', !1), $('.submit-tr .submit-btn').css({
            cursor: 'pointer'
        })) : ($('#js-sxbz').html('您未达到国家线要求，只能按规定申请校内调剂。'), $('.submit-tr .submit-btn').attr('disabled', !0), $('.submit-tr .submit-btn').css({
            cursor: 'not-allowed'
        })),
        init_ajax(Mainfest['tbtjzy-yxxx'].interface_url + '?code=' + dw_code, function(o) {
            $('#yxxx').html(return_html(f, o, yx_code, '2')),
                o.length > 0 && '' != o[0].bz ? ($('#edit-time').text(o[0].bz), $('.dwxx-web').addClass('mar-b0'), $('.tb-tip-time').show()) : ($('.dwxx-web').removeClass('mar-b0'), $('.tb-tip-time').hide()),
                f ? seach_zyxx(!0) : ($('#adjust_id').val(''), $('#zyxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '专业' : '') + '</option>'), $('#fxxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '研究方向' : '') + '</option>'), $('#xxfs').html('<option value="">请选择' + ('wap' == mark.agent_from ? '学习方式' : '') + '</option>'))
        })
}
...
function seach_zyxx(f) {
    ...
    var dw_code = $('#dwxx').val(),
        yx_code = $('#yxxx').val(),
        zy_code = $('#zyxx').val();
    init_ajax(Mainfest['tbtjzy-zyxx'].interface_url + '?code=' + dw_code + '&code1=' + yx_code, function(o) {
        $('#zyxx').html(return_html(f, o, zy_code, '3')),
            f && '' != $('#zyxx').val() ? seach_fxxxx(!0) : ($('#adjust_id').val(''), $('#fxxxx').html('<option value="">请选择' + ('wap' == mark.agent_from ? '研究方向' : '') + '</option>'), $('#xxfs').html('<option value="">请选择' + ('wap' == mark.agent_from ? '学习方式' : '') + '</option>'))
    })
}
...
function seach_fxxxx(f) {
    ...
    var dw_code = $('#dwxx').val(),
        ...
        zy_code = $('#zyxx').val(),
        fx_code = $('#fxxxx').val();
    init_ajax(Mainfest['tbtjzy-fxxxx'].interface_url + '?code=' + dw_code + '&code1=' + yx_code + '&code2=' + zy_code, function(o) {
        $('#fxxxx').html(return_html(f, o, fx_code, '4')),
            f && '' != $('#fxxxx').val() ? seach_xxfs(!0) : ($('#adjust_id').val(''), $('#xxfs').val('<option value="">请选择' + ('wap' == mark.agent_from ? '学习方式' : '') + '</option>'))
    })
}
...
function seach_xxfs(f) {
    ...
    var dw_code = $('#dwxx').val(),
        ...
        fx_code = $('#fxxxx').val(),
        fs_code = $('#xxfs').val();
    init_ajax(Mainfest['tbtjzy-xffs'].interface_url + '?code=' + dw_code + '&code1=' + yx_code + '&code2=' + zy_code + '&code3=' + fx_code, function(o) {
        bz_Array = o,
            $('#xxfs').html(return_html(f, o, fs_code, '5')),
            '' == bz_Array && '' != fx_code ? $('#fxxxx').siblings('.tj-error').html('<i class="iconfont">&#xe67c;</i>该研究方向无计划余额') : ($('#fxxxx').siblings('.tj-error').html(''), seach_bz())
    })
}
...
function seach_bz() {
    ...
    for (var _html = '', i = 0; i < bz_Array.length; i++)
        if (fs_code == bz_Array[i].code) {
            var zy_id = bz_Array[i].id;
            return '' != zy_id && (_html = '<a onclick="template_zytb_tj(\'' + zy_id + '\')">查看申请条件</a>'),
                $('#tsyq').html(_html),
                void $('#adjust_id').val(bz_Array[i].id)
        }
}
...
function change_seach(sysType, seachType, e) {
    ...
    $('#tj_seach_change').replaceWith(template(templateName, {
            seachType: seachType
        })),
        init_select_ss(mark.agent_from)
}
...
function menuHighlight() {
    ...
    if (navfest[_url]) {
        ...
        _obj.hasClass('act') || _obj.addClass('act'),
            _obj.siblings('li').removeClass('act');
        var _wapObj = $('.tj-menu-wap a[data-nav=' + _nav + ']');
        _wapObj.hasClass('act') || _wapObj.addClass('act'),
            _wapObj.siblings().removeClass('act'),
            $.each($('.tj-menu-wap a'), function(i, it) {
                i < _wapObj.index() && (curL += $(it).width())
            }),
            $('.tj-menu-wap').animate({
                scrollLeft: curL
            }, 100)
...
function modifyHeight(className, parentClassName) {
    ...
    $('.' + className).each(function() {
        ...
        $('.' + className).css('height', _array.max()),
        $('.' + parentClassName).removeClass(parentClassName)
}
...
function formserialize(element) {
    return $(element).parents('form').serializeArray()
}
...
function template_zytb_tj(id) {
    init_ajax(Mainfest['tj-sqtj'].interface_url + '?id=' + id, function(data) {
        var _html = template(Mainfest['tj-sqtj'].template, data),
            _area = (data.other.agent_from, [
                'auto',
                'auto'
            ]);
        layer.alert(_html, {
            id: 'info-alert',
            title: '<span class="color-blue" style="font-size: 16px;margin-right: 10px;">申请条件</span>',
            area: _area,
            scrollbar: !1
        })
    })
...
function randerTemplate(interfaceName, data) {
    return template(Mainfest[interfaceName].template, data)
}

function common_global_loading() {
    $('#loading').show()
}

function common_global_unloading() {
    $('#loading').hide()
}

function init_layout(agent_from, interfaceName) {
    $('body').prepend(template('common/layout', {
            agent_from: agent_from
        })),
        init_notice(agent_from, interfaceName)
}

function init_layout_tjyx(agent_from, interfaceName) {
    $('body').prepend(template('common/tjyx-layout', {
            agent_from: agent_from
        })),
        init_notice(agent_from, interfaceName)
}

function init_interval(agent_from, interfaceName, sysType) {
    window.setInterval(function() {
        init_ajax(Mainfest['notice-init'].interface_url + '?systype=' + sysType, function(data) {
            init_fs_dlq(sysType, data)
        })
    }, 300000)
}
...
function init_notice(agent_from, interfaceName) {
    var sysType = 'tjyx' == interfaceName.substring(0, 4) ? 'TJYX' : 'TJ',
        isshow = !1;
    if ('cjxx' == interfaceName || 'yzytz' == interfaceName) {
        var _agent = '?agent_from=' + agent_from;
        return void init_ajax(Mainfest[interfaceName].interface_url + _agent, function(yzydata) {
            $('#yz_userid').html(yzydata.uname),
                yzydata.yzy_usable ? yzydata.flag ? ($('#content').html(randerTemplate(interfaceName, yzydata)), menuHighlight()) : $('#content').html(template('common/unuse', {
                    page_error: yzydata.message.page_error
                })) : $('#content').append(template('common/sys', {
                    sysData: yzydata.yzy_usable_des
                }))
        })
    }
    'tjyx-index' != interfaceName && 'index' != interfaceName || ($('#content').append(template(Mainfest[interfaceName].template, {
            agent_from: agent_from
        })), isshow = !0),
        init_ajax(Mainfest['notice-init'].interface_url + '?systype=' + sysType, function(data) {
            if (mark = data.other, $('#yz_userid').html(data_cookie(data)), isshow) $('#content').prepend(template('common/notice', {
                    basedata: data,
                    sysType: sysType
                })),
                menuHighlight(),
                init_fs_dlq(sysType, data);
            else if (init_sys_open(sysType, data), sysIsOpen) {
                if (init_isuse(sysType, data), isUse) return init_layout_interface(agent_from, interfaceName),
                    void init_fs_dlq(sysType, data);
                return
            }
        })
}
...
function init_fs_dlq(sysType, o) {
    if ('TJ' == sysType) {
        ...
        if ('' != tz_array)
            for (var i = 0; i < tz_array.length; i++) 'fs' == tz_array[i].type && (fs_tz = !0),
                'lq' == tz_array[i].type && (dlq_tz = !0);
        fs_tz ? flag ? $('.tj-menu-wap a[data-nav="fstz"] span').show() : $('.tj-menu li[data-nav="fstz"] span').show() : flag ? $('.tj-menu-wap a[data-nav="fstz"] span').hide() : $('.tj-menu li[data-nav="fstz"] span').hide(),
            dlq_tz ? flag ? $('.tj-menu-wap a[data-nav="dlqtz"] span').show() : $('.tj-menu li[data-nav="dlqtz"] span').show() : flag ? $('.tj-menu-wap a[data-nav="dlqtz"] span').hide() : $('.tj-menu li[data-nav="dlqtz"] span').hide()
    }
}

function init_sys_open(sysType, o) {
    'TJYX' == sysType ? (sysIsOpen = o.tjyx_usable, o.tjyx_usable || $('#content').append(template('common/sys', {
        sysData: o.tjyx_usable_des
    }))) : (sysIsOpen = o.tj_usable, o.tj_usable || $('#content').append(template('common/sys', {
        sysData: o.tj_usable_des
    })))
}

function init_isuse(sysType, o) {
    var flag = 'TJYX' == sysType;
    flag ? _sxbz = o.tjyxsxbz : (_sxbz = o.sxbz, sxbz2 = o.sxbz2, sxbz3 = o.sxbz3, ksbh = o.ksbh),
        1 != _sxbz && ('1' == o.sxbz2 || '1' == o.sxbz3 || (isUse = !1, flag ? $('#content').html(template('common/tjyx-unuse', {
            _sxbz: _sxbz,
            userid: o.userid,
            _agent_from: o.other.agent_from,
            page_error: o.message.page_error
        })) : $('#content').html(template('common/unuse', {
            _sxbz: _sxbz,
            userid: o.userid,
            _agent_from: o.other.agent_from,
            page_error: o.message.page_error
...
function init_layout_interface(agent_from, interfaceName) {
    if (menuHighlight(), 'tjyx-qecx' == interfaceName || 'qecx' == interfaceName) return void $('#content').append(template(Mainfest[interfaceName].template, {
        agent_from: agent_from,
        seachType: 'accurate',
        sxbz2: sxbz2,
        sxbz3: sxbz3
    ...
    init_ajax(host + Mainfest[interfaceName].interface_url + _agent + _hasId, function(o) {
        init_page(interfaceName, 'content', o),
            'mytjzy' != interfaceName && 'tjyx-mytjyx' != interfaceName || (modifyHeight('zy-main', 'zy-main-postion'), zy_status = o.data.notice.status, zy_status.cur_num < zy_status.max_num && $(function() {
                $('.ewm-wap').addClass('ewm-wap-top')
            }))
    })
}

function init_page(interfaceName, element, data) {
    if (data.flag) {
        if ('tjyx-qecx' == interfaceName) return void $('#content').html(template('common/tjyx-qecx', {
            agent_from: agent_from
        }));
        if ('qecx' == interfaceName) return void $('#content').html(template('common/qecx', {
            agent_from: agent_from,
            sxbz2: sxbz2,
            sxbz3: sxbz3
        }));
        'qecx-list' == interfaceName || 'tjyx-qecx-list' == interfaceName || 'qecx-list-wap' == interfaceName || 'tjyx-qecx-list-wap' == interfaceName ? $('#' + element).replaceWith(randerTemplate(interfaceName, data)) : ($('#' + element).html(randerTemplate(interfaceName, data)), 'tbtjzy-edit' == interfaceName && seach_ssdm(!0))
    } else {
        if ('tbtjzy' == interfaceName || 'tjyx-tbtjzy' == interfaceName || 'unuse' == interfaceName || 'tjyx-unuse' == interfaceName) return void $('#' + element).html(randerTemplate(interfaceName, data));
        '' != data.message.page_error ? layer.alert(data.message.page_error, {
            title: '提示'
        }, function(index) {
            'qecx-list' == interfaceName || 'tjyx-qecx-list' == interfaceName || 'qecx-list-wap' == interfaceName || 'tjyx-qecx-list-wap' == interfaceName ? layer.close(index) : window.location.reload()
        }) : '' != data.message.error && layer.alert(data.message.error[0].mc, {
            title: '提示'
        }, function(index) {
            'qecx-list' == interfaceName || 'tjyx-qecx-list' == interfaceName || 'qecx-list-wap' == interfaceName || 'tjyx-qecx-list-wap' == interfaceName ? layer.close(index) : 'tbtjzy-edit' == interfaceName ? window.location.href = 'tjzy.html' : window.location.reload()
        })
    }
}

function data_cookie(o) {
    return null != $.cookie('userName') ? $.cookie('userName') : ($.cookie('userName', o.uname), $.cookie('userId', o.userid), o.uname)
}
...
function init_ajax(url, callbackfun) {
    $.ajax({
        ...
        complete: function(XMLHttpRequest, textStatus) {
            '400' == XMLHttpRequest.status && (common_global_unloading(), layer.alert('访问过于频繁，请稍后再试。', {
                    closeBtn: 0,
                    title: '提示'
                }, function(index) {
                    layer.close(index)
                })),
                '403' == XMLHttpRequest.status && (common_global_unloading(), layer.alert('操作失败，请检查输入的字符或操作是否合法。', {
                    closeBtn: 0,
                    title: '提示'
                }, function(index) {
                    layer.close(index)
...
function post_ajax(url, postdata, callbackfun) {
    $.ajax({
        type: 'post',
        url: url,
        traditional: !1,
        ...
        data: other_change(postdata),
        success: function(callbackdata) {
            chose_ajax('post', url, callbackdata, callbackfun, postdata)
        },
        complete: function(XMLHttpRequest, textStatus) {
            '400' == XMLHttpRequest.status && (common_global_unloading(), layer.alert('访问过于频繁，请稍后再试。', {
                    closeBtn: 0,
...
function chose_ajax(type, url, callbackdata, callbackfun, postdata) {
    if ('object' != typeof callbackdata) {
        if (16 == $.trim(callbackdata).length) {
            var polling = new AjaxPolling(callbackdata, '', '/sytj/asynprogress.do', {
                retryLimit: 4,
                delayGaps: 300
            });
            return polling.onSuccess = function(pollingData) {
                    return 'get' == type ? void init_ajax(url + '&taskId=' + callbackdata, callbackfun) : void post_ajax(url + '?taskId=' + callbackdata, postdata, callbackfun)
                },
                polling.onTimeOut = function() {
        ...
        return common_global_unloading(),
            void layer.alert('请求人数过多，请稍后再试。', {
                title: '提示'
            }, function(index) {
                layer.close(index)
            })
...
function error_ajax(XMLHttpRequest, textStatus, errorThrown) {
    common_global_unloading(),
        'timeout' == textStatus ? layer.alert('请求超时，请稍后再试。', {
            title: '提示'
        }, function(index) {
            layer.close(index)
        }) : 'error' == textStatus && layer.alert('操作已过时，请 <a href=\'https://yz.chsi.com.cn/user/\'>重新登录</a>。', {
            closeBtn: 0,
            title: '提示'
        }, function(index) {
            layer.close(index)
...
function template_bdzkzh(interfaceName, element, e) {
    ...
    post_ajax(Mainfest[interfaceName].interface_url, form_data, function(o) {
        if (o.flag) layer.alert('准考证绑定成功', {
            closeBtn: 0,
            title: '提示'
        }, function() {
            window.location.href = 'cjxx.html'
        });
        else {
...
function template_fstz(interfaceName, data, element) {
    var warn_text = '确实要接受吗？确定后不能更改。';
    'fstz-post-reject' == interfaceName && (warn_text = '确实要拒绝吗？请慎重，确定后不能更改。'),
        layer.confirm(warn_text, {
            btn: [
                '确定',
                '取消'
            ],
            ...
            layer.close(index),
                post_ajax(Mainfest[interfaceName].interface_url, {
                    id: data
                }, function(o) {
                    init_page(interfaceName, element, o)
                })
        }, function(index) {
            return layer.close(index), !1
        })
}

function template_dlqtz(interfaceName, data, element) {
    var warn_text = '考生一经确认，非经招生单位同意不得调剂录取至其他招生单位，确定要接受？';
    'dlqtz-post-reject' == interfaceName && (warn_text = '确实要拒绝吗？请慎重，确定后不能更改。'),
        layer.confirm(warn_text, {
            btn: [
                '确定',
                '取消'
...
function template_dlqtz_cancel(interfaceName, data, element) {
    var warn_text = '确实要接受吗？确定后不能更改。';
    'dlqtz-post-rejectcancel' == interfaceName && (warn_text = '确实要拒绝吗？'),
        layer.confirm(warn_text, {
            btn: [
                '确定',
                '取消'
...
function template_dlqtz_cancel_yzy(interfaceName, data, element) {
    ...
    'yzy-dlqtz-cancel-reject' == interfaceName && (warn_text = '确实要拒绝吗？'),
        layer.confirm(warn_text, {
            btn: [
                '确定',
                '取消'
...
function template_zytb_edit(id) {
    window.location.href = 'tbtjzy.html?zy_type=2&id=' + id
}
...
function template_zytb_submit(interfaceName, element, e) {
    ...
    layer.confirm('请确认您填报的' + sysmsg + '是否符合教育部招生工作管理规定和招生单位报考条件及要求，提交之后' + confirmTime + '确定要提交吗？', {
        btn: [
            '确定',
            '取消'
        ],
        ...
        var form_data = formserialize(element);
        post_ajax(Mainfest[interfaceName].interface_url, form_data, function(o) {
            template_zytb_callback(o, interfaceName)
        })
    })
}
...
function template_zytb_callback(data, interfaceName) {
    ...
    if (data.flag) 'TJYX' == sysType ? $('#content').html(template('common/tjyx-success', {
        agent_from: agent_from
    })) : $('#content').html(template('common/success', {
        agent_from: agent_from,
        zy_type: interfaceName
    }));
    else {
        for (var _errorMessage = data.message.error, _html = '', i = 0; i < _errorMessage.length; i++) {
            var _obj = _errorMessage[i];
            'yddh' == _obj.dm ? $('#yddh').focus().siblings('.tj-error').html('<i class="iconfont">&#xe67c;</i>' + _obj.mc) : 'bz' == _obj.dm ? $('#bz').focus().siblings('.tj-error').html('<i class="iconfont">&#xe67c;</i>' + _obj.mc) : ($('#yddh').siblings('.tj-error').html(''), $('#bz').siblings('.tj-error').html(''), 'jybtjzz' == _obj.dm && (_obj.mc = '<a target="_blank" href="https://yz.chsi.com.cn/kyzx/tjzd/202103/20210305/2035189760.html">' + _obj.mc + '</a>'), 'wap' == data.other.agent_from ? _html += '<tr class="tj-error-tr"><td><div class="tj-error-global"><i class="iconfont">&#xe67c;</i>' + _obj.mc + '</div></td></tr>' : _html += '<tr class="tj-error-tr"><td></td><td colspan="2"><div class="tj-error-global"><i class="iconfont">&#xe67c;</i>' + _obj.mc + '</div></td></tr>')
        }
...
function pageclick(page) {
    ...
    $('#start').val(page_start),
        ...
        qecx_post($('#sysname').val(), $('#tj_seach_form').serializeArray())
}

function test_seach() {
    var dwxxValue = $('#dwxx').val(),
        zyxxValue = $('#zyxx').val();
    return '' != dwxxValue || '' != zyxxValue || (layer.alert('招生单位和专业至少填写其中一项', {
        title: '提示'
    }, function(index) {
        layer.close(index)
    }), !1)
...
function test_seach_fuzzy() {
    return !($.trim($('#dwxx').val()).length < 2 && (layer.alert('招生单位或专业名称的关键词长度至少为2个字符', {
        title: '提示'
    }, function(index) {
        layer.close(index)
    }), 1))
...
function qecx_post(interfaceName, form_data) {
    post_ajax(Mainfest[interfaceName].interface_url, form_data, function(o) {
        mark = o.other,
            o.flag ? (zy_status = o.data.notice.status, init_page(interfaceName, 'content-qecxList', o), 'qecx-list-wap' != interfaceName && 'tjyx-qecx-list-wap' != interfaceName || (init_scoll(), pagenation = o.data.vo_list.pagenation), rander_order()) : layer.alert(o.message.page_error, {
                title: '提示'
            }, function(index) {
                layer.close(index)
            })
...
function order_qelist() {
    $('body').on('click', '.sort-icon', function() {
        ...
        '' != _value ? $('#order_by').val(_name + '-' + _value) : $('#order_by').val(''),
            template_qecx_sort()
    })
}

function template_qecx_sort() {
    $('#start').val(''),
        $('#page_size').val(page_size),
        qecx_post($('#sysname').val(), $('#tj_seach_form').serializeArray())
}
...
function rander_order() {
    ...
    if ('' != str) {
        ...
        $('.sort-icon[data-name="' + strArry[0] + '"]').removeClass('asc').removeClass('desc').addClass(strArry[1]).attr('data-value', strArry[1]),
            $('.sort-icon[data-name!="' + strArry[0] + '"]').removeClass('asc').removeClass('desc').attr('data-value', '')
    } else $('.sort-icon').removeClass('asc').removeClass('desc').attr('data-value', '')
}

function template_totjzy(interfaceName, id, e) {
    preventDefaultEvent(e),
        layer.confirm('确定要将调剂意向转为调剂志愿吗？', {
            btn: [
                '确定',
                '取消'
            ],
            ...
            layer.close(index),
                post_ajax(Mainfest[interfaceName].interface_url, {
                    ...
                    o.flag ? (init_page('tjyx-mytjyx', 'content', o), modifyHeight('zy-main', 'zy-main-postion')) : '' != o.message.page_error ? layer.alert(o.message.page_error, {
                        title: '提示'
                    }, function(index) {
                        layer.close(index)
                    }) : layer.alert(o.message.error[0].mc, {
                        title: '提示'
                    }, function(index) {
                        layer.close(index)
                    })
...
function init_scoll() {
    window.wapLoaded = !0,
        window.onscroll = function() {
            ...
            window.wapLoaded && scrollTop / (contentH - viewH) >= 0.95 && (window.wapLoaded = !1, Load())
        }
}
...
function Load() {
    ...
    if (_nextpage <= Math.ceil(pagenation.totalCount / pagenation.pageCount)) {
        var interfaceName,
            el = $('#wrapper ul'),
            sysName = mark.pageid;
        interfaceName = 'tjyx_qe_list' == sysName ? 'tjyx-qecx-list' : 'qecx-list',
            post_ajax(Mainfest[interfaceName].interface_url, form_data, function(o) {
                window.wapLoaded = !0,
                    'tjyx-qecx-list' == interfaceName ? (el.append(template('wap/tjyx-qecx-list-item', o)), pagenation = o.data.vo_list.pagenation) : (el.append(template('wap/qecx-list-item', o)), pagenation = o.data.vo_list.pagenation)
            })
    }
}
...
function acceptJs(id, jsyy) {
    layer.confirm('该调剂志愿将无效，可继续填报其他志愿。', {
        btn: [
            '确定',
            '取消'
        ],
        ...
        layer.close(index),
            post_ajax('/sytj/stu/agreeapplyjs.action', {
                id: id
            }, function(o) {
                init_page('mytjzy', 'content', o),
                    modifyHeight('zy-main', 'zy-main-postion')
            })
    }, function(index) {
        return layer.close(index), !1
    })
...
function rejectJs(id, jsyy) {
    layer.confirm('该调剂志愿锁定时间到达后方可继续填报其他志愿。', {
        btn: [
            '确定',
            '取消'
        ],
        ...
        layer.close(index),
            post_ajax('/sytj/stu/disagreeapplyjs.action', {
                id: id
            }, function(o) {
                init_page('mytjzy', 'content', o),
                    modifyHeight('zy-main', 'zy-main-postion')
...
function html2Escape(sHtml) {
    ...
    function compile(filename, fn) {
        ...
        return render.prototype = fn.prototype = utils,
    ...
    var cache = template.cache = {},
        ...
        utils = template.utils = {
            ...
            $include: function(filename, data, from) {
                return filename = resolve(from, filename),
    ...
    template.get = function(filename) {
        ...
        template.helper('changeLetter', function(data) {
            return String.fromCharCode(data + 65)
        }),
        template.helper('defValue', function(data, v) {
            return '' != data && data ? data : v
        }),
        ...
        template.helper('pagehelp', function(data) {
            return Math.ceil(data)
        }),
        template.helper('pagerander', function(data) {
            var totalPage = Math.ceil(data.totalCount / data.pageCount),
                curPage = data.curPage;
            return function(count, pageindex) {
                function setPageList() {
                    a[a.length] = pageindex == i ? '<li class="tj-paging-item tj-paging-check"><a href="###" class="on" onclick="pageclick(' + i + ')">' + i + '</a></li>' : '<li class="tj-paging-item"><a href="###" onclick="pageclick(' + i + ')">' + i + '</a></li>'
                }
                var a = [];
                if (a[a.length] = 1 == pageindex ? '<li class="tj-paging-item tj-paging-disabled"><a href="###" class="prev unclick">上一页</a></li>' : '<li class="tj-paging-item"><a href="###" class="prev" onclick="pageclick(' + (curPage - 1) + ')">上一页</a></li>', count <= 10)
                    ...
                    for (var i = count - 4; i <= count; i++) setPageList()
                } else {
                    a[a.length] = '<li class="tj-paging-item"><a href="###" onclick="pageclick(1)">1</a></li><li class="tj-paging-item"><a href="###">...</a></li>';
                    for (var i = pageindex - 2; i <= pageindex + 2; i++) setPageList();
                    a[a.length] = '<li class="tj-paging-item"><a href="###">...</a></li><li class="tj-paging-item"><a href="###" onclick="pageclick(' + count + ')">' + count + '</a></li>'
                }
                return a[a.length] = pageindex == count ? '<li class="tj-paging-item tj-paging-disabled"><a href="###" class="next unclick">下一页</a></li>' : '<li class="tj-paging-item"><a href="###" class="next" onclick="pageclick(' + (curPage + 1) + ')">下一页</a></li>',
                    a.join('')
            }(totalPage, curPage)
        }),
        ...
        template('common/index', function($data, $filename) {
            ...
            return 'wap' == agent_from && ($out += ' <div class="enter-wap"> <div class="enter-wap-item"> <div class="enter-wap-item-left">一志愿</div> <div class="enter-wap-item-right"> <a href="yzytz.html">一志愿复试待录取通知</a> </div> </div> <div class="enter-wap-item"> <div class="enter-wap-item-left">调剂</div> <div class="enter-wap-item-right flex"> <a href="qecx.html">计划余<br>额查询</a> <a href="tjzy.html">申请<br>调剂</a> <a href="fstz.html">复试<br>通知</a> <a href="dlqtz.html">待录取<br>通知</a> </div> </div> <div class="enter-wap-item"> <div class="enter-wap-item-left">初试</div> <div class="enter-wap-item-right"> <a href="cjxx.html">初试成绩</a> </div> </div> </div> <div class="index-ewm-wap"> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/chsi.jpg" /> <div class="ewm-title">学信网公众号</div> <div class="ewm-con">关注学信网公众号并绑定微信账号，及时接收重要消息提醒</div> </div> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/yz.jpg" /> <div class="ewm-title">研招网</div> <div class="ewm-con">研招关键节点提醒及相关信息查询</div> </div> </div> '),
                ...
                $out += ' ',
                'wap' == agent_from && ($out += ' <h3 class="index-h2" style="margin-top: 0;">调剂须知</h3> '),
                $out += ' <div class="sys-tip-ktsj"> <div class="sys-tip-warp ',
                'web' == agent_from && ($out += 'sys-tip-bg'),
                $out += '"> <div class="sys-tip-title">调剂系统开通时间</div> <div class="sys-tip-time">4月6日</div> </div> <p>各招生单位调剂工作开展日期、相关调剂要求和计划余额信息由招生单位根据教育部相关规定自行确定并公布，详细情况请考生咨询有关招生单位。</p> </div> <div class="tip ',
                'web' == agent_from && ($out += 'tip-web'),
                $out += '"> <h4 class="tip-title">重要提示</h4> <div class="tip-content"> <a href="https://yz.chsi.com.cn/kyzx/tjzd/202203/20220311/2172264654.html" target="_blank">基本要求</a> ',
                'wap' == agent_from && ($out += '<span class="cut-l"></span>'),
                $out += ' <!-- <a href="https://yz.chsi.com.cn/kyzx/kp/202103/20210319/1773474721.html" target="_blank">操作流程</a> ',
                'wap' == agent_from && ($out += '<span class="cut-l"></span>'),
                $out += ' --> <a href="https://yz.chsi.com.cn/help/index.jsp" target="_blank">常见问题</a> ',
                'wap' == agent_from && ($out += '<span class="cut-l"></span>'),
                $out += ' <a href="https://yz.chsi.com.cn/kyzx/tjzd/202203/20220329/2176565576.html" target="_blank" style="margin-right:0">注意事项</a> </div> </div> </div> ',
                $out += 'web' == agent_from ? ' <div class="vote-part mar-t20"> <span class="vote-part-tip">请进行网上调查</span><span class="vote-part-time">（大约1分钟）</span> <a class="vote-part-dc" href="https://vote.chsi.com.cn/vote/real/showWholeVote.action?groupId=5rwvp9jq83kvl650" target="_blank">网上调查</a> <a class="vote-part-cp" href="https://xz.chsi.com.cn/cp/sph/cover.do?sphSurveyId=f4ediwycn6qj8nm4" target="_blank">专业认同度测评</a> </div> ' : ' <div class="cp-warp"> <a href="https://vote.chsi.com.cn/vote/real/showWholeVote.action?groupId=5rwvp9jq83kvl650"> <img src="https://t2.chei.com.cn/yz/sytj/assets/images/common/index-wsdc.png" alt=""> </a> <a href="https://xz.chsi.com.cn/cp/sph/cover.do?sphSurveyId=f4ediwycn6qj8nm4"> <img src="https://t2.chei.com.cn/yz/sytj/assets/images/common/index-cp.png" alt=""> </a> </div> ',
                $out += ' <div class="wap-index-h2"> <h2 class="index-h2 mar-t40">操作流程</h2> </div> ',
                'web' == agent_from && ($out += ' <div class="tj-step-icon"> <span class="num first">1</span> <span class="cutline"></span> <span class="num">2</span> <span class="cutline"></span> <span class="num last">3</span> </div> '),
                $out += ' <div class="wap-expend-warp" id="wap-expend-warp"> <ul class="tj-step"> <li> <h4>',
                'wap' == agent_from && ($out += '<span class="step-num">1</span>'),
                $out += '报名</h4> <p><span class="step-strong">登录：</span>考生凭网报时注册的用户名和密码登录“中国研究生招生信息网”的网上调剂系统。如果忘记了用户名或密码，请使用“忘记密码”功能，输入相关信息来找回用户名或密码。</p> <p><span class="step-strong">查询计划余额：</span>考生填报调剂志愿前，请务必充分了解教育部调剂相关政策及招生单位（含各院、系、所）的调剂办法、计划余额、接收考生调剂时间、基本要求、工作程序、调剂复试办法等信息，登录调剂系统查询各招生单位的计划余额信息、调剂办法、接收考生调剂的申请条件。特别注意：申请调剂的考生应符合招生管理规定确定的调剂条件。不符合调入专业学科门类要求、初试科目要求，或不满足招生单位调剂条件的考生，将无法申请填报相应志愿。</p> <p><span class="step-strong">填报调剂志愿：</span>通过调剂服务系统选择已发布计划余额的招生单位和专业研究方向，如满足教育部调剂政策且符合招生单位设置的调剂报考条件，可填报调剂志愿（可一次填报三个平行调剂志愿），每个调剂志愿提交后即由招生单位锁定，锁定时间由各招生单位自主设定，最长不超过36小时。招生单位根据本单位调剂工作要求和生源情况确定是否接收考生参加复试。如考生调剂申请未获招生单位通过，调剂志愿解锁，考生可继续填报其他志愿。锁定时间到达后，系统自动解锁，考生可继续等待或改填报其他志愿。</p> </li> <li> <h4>',
                'wap' == agent_from && ($out += '<span class="step-num">2</span>'),
                $out += '复试通知</h4> <p>考生提交调剂志愿后，如招生单位同意其参加复试，将通过调剂系统发送复试通知。请及时登录系统查看志愿状态及是否接到复试通知。如接到复试通知，请考生及时通过调剂系统确认回复，并按招生单位要求办理相关手续。复试没有通过的考生仍可继续填报其他加调剂志愿。</p> </li> <li> <h4>',
                'wap' == agent_from && ($out += '<span class="step-num">3</span>'),
                $out += '待录取通知</h4> <p>复试通过后，招生单位将通过调剂系统给考生发送待录取通知，接到待录取通知的考生应在招生单位规定时间内登录调剂系统进行确认，否则招生单位可取消待录取通知。考生一旦确认接受待录取通知，表示调剂完成，将不能再填报调剂志愿、接受复试或待录取通知。考生如取消已确认的待录取通知，必须征得招生单位同意，由招生单位取消待录取通知，且考生登录调剂系统进行确认后，方可继续填报调剂志愿知。</p> </li> </ul> ',
                'wap' == agent_from && ($out += ' <div class="wap-expend"> <div class="wap-expend-bg"></div> <div class="wap-expend-text text-expend"> <span>展开</span> <i class="iconfont">&#xe60f;</i> </div> <div class="wap-expend-text text-unexpend"> <span>收起</span> <i class="iconfont">&#xe608;</i> </div> </div> '),
                $out += ' </div> ',
                'wap' == agent_from && ($out += ' <div class="ewm-wap"> <i class="iconfont" title="二维码">&#xe65b;</i> </div> <div class="wap-overlay tj-hide"></div> <div class="ewm-detail-wap tj-hide"> <div class="item-con"> <div class="item"> <div class="left"> <img src="//t1.chei.com.cn/common/ewm/images/chsi.jpg" /> </div> <div class="right"> <div class="ewm-title">学信网公众号</div> <div class="ewm-con">关注学信网公众号并绑定微信账号，及时接收重要消息提醒</div> </div> </div> <div class="item"> <div class="left"> <img src="//t1.chei.com.cn/common/ewm/images/yz.jpg" /> </div> <div class="right"> <div class="ewm-title">研招网</div> <div class="ewm-con">研招关键节点提醒及相关信息查询</div> </div> </div> </div> <i class="iconfont close">&#xe65c;</i> </div> '),
                new String($out)
        }),
        template('common/layout', function($data, $filename) {
            'use strict';
            var $utils = this,
                include = ($utils.$helpers, function(filename, data) {
                    data = data || $data;
                    var text = $utils.$include(filename, data, $filename);
                    return $out += text
                }),
                $out = '';
            return $out += ' <div class=\'tj-main\'> <div class=\'tj-content\'> ',
                include('../common/header'),
                $out += ' <div class="width1180" id="content"> </div> </div> </div> ',
        ...
        template('common/notice', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = ($data.index, '');
            return $out += ' ',
                '' != basedata.other.flash.tips && ($out += '  <!-- <div class="info-prompt mar-t30"><span class="tj-icon-warn"><i class="iconfont">&#xe674;</i></span> <ul> ', $each(basedata.other.flash.tips, function(value, inde) {
                    $out += ' <li>',
                        value.escape ? $out += $string(value.title) : $out += $escape(value.title),
                        $out += '</li> '
                }), $out += ' </ul> </div> --> '),
                $out += ' ',
                'TJ' == sysType ? ($out += '  ', '' != basedata.data.notice.news && ($out += ' <!-- <div class="sys-tip index-sys-tip mar-t20"> <h2><i class="iconfont" title="通知">&#xe6a7;</i>通知</h2> <ul class="sys-list"> ', $each(basedata.data.notice.news, function(value, index) {
                    $out += ' <li><span></span>',
                        $out += $string(value.title),
                        'fs' == value.type ? $out += '&ensp;<a href="fstz.html">去看看</a>' : 'lq' == value.type && ($out += '&ensp;<a href="dlqtz.html">去看看</a>'),
                        $out += '</li> '
                }), $out += ' </ul> </div> --> '), $out += ' ') : ($out += ' ', 'wap' == basedata.other.agent_from ? ($out += ' <div class="enter-wap"> <div class="enter-wap-item"> <div class="enter-wap-item-left">意向</div> <div class="enter-wap-item-right flex"> <a href="qecx.html" class="flex-1">调剂意向余额查询</a> <a href="tjzy.html" class="flex-2">调剂意向</a> </div> </div> <div class="enter-wap-item"> <div class="enter-wap-item-left">初试</div> <div class="enter-wap-item-right"> <a href="cjxx.html">初试成绩</a> </div> </div> </div> <div class="index-ewm-wap"> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/chsi.jpg" /> <div class="ewm-title">学信网公众号</div> <div class="ewm-con">关注学信网公众号并绑定微信账号，及时接收重要消息提醒</div> </div> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/yz.jpg" /> <div class="ewm-title">研招网</div> <div class="ewm-con">研招关键节点提醒及相关信息查询</div> </div> </div> ', basedata.tj_usable && ($out += ' <div class="index-entry text-c"> <a href="http://yz.chsi.com.cn/sytj/tj/" class="entry-btn" >网上调剂服务系统</a> </div> '), $out += ' ') : ($out += ' <div class="index-tjxz mar-t20"> <div class="index-ewm clearfix"> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/chsi.jpg" /> <div class="ewm-title">学信网公众号</div> <div class="ewm-con">关注学信网公众号并绑定微信账号，<br>及时接收重要消息提醒</div> </div> <div class="item"> <img src="//t1.chei.com.cn/common/ewm/images/yz.jpg" /> <div class="ewm-title">研招网</div> <div class="ewm-con">研招关键节点提醒及相关信息查询</div> </div> </div> ', basedata.tj_usable && ($out += ' <div class="index-entry text-c"> <a href="http://yz.chsi.com.cn/sytj/tj/" class="entry-btn" >网上调剂服务系统</a> </div> '), $out += ' </div> '), $out += ' '),
                $out += ' ',
                new String($out)
        }),
        template('common/qecx-quto', ' <div class="tj-elem-quto mar-t10"> <div class="tj-elem-bg"></div> <div class="tj-elem-content">请选择或填写条件进行计划余额查询</div> </div> '),
        template('common/qecx-seach-item', function($data, $filename) {
            'use strict';
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                $each(seachList, function(value, index) {
                    $out += ' <option value="',
                        $out += $escape(value.code),
                        $out += '" ',
                        selectitem && selectitem == value.code && ($out += 'selected="selected"'),
                        $out += '>(',
                        $out += $escape(value.code),
                        $out += ') ',
        ...
        template('common/qecx', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                '1' != sxbz2 && '1' != sxbz3 || '1' == sxbz || ($out += ' ', include('../common/sxbz-include'), $out += ' '),
                $out += ' ',
                'web' == agent_from ? ($out += ' ', include('../web/qecx-seach'), $out += '  <div id="content-qecxList"> ', include('../common/qecx-quto'), $out += ' </div> ', include('../common/tips'), $out += '  ') : ($out += ' ', include('../wap/qecx-seach'), $out += '  <div id="content-qecxList"> ', include('../common/tips'), $out += '  </div> '),
                $out += ' <script> init_select_ss(\'',
                $out += $escape(agent_from),
                $out += '\'); </script> ',
                new String($out)
        }),
        ...
        template('common/success', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += '<div class="tj-nopermission"> <div class="tj-icon-success"></div> <h2>',
                $out += 'tbtjzy-edit-post' == zy_type ? '志愿修改已完成!' : '志愿填报已完成！',
                $out += '</h2> <p class="tj-success-p"> ',
                $out += 'tbtjzy-edit-post' == zy_type ? ' ' : ' <a href="qecx.html">继续填报</a> | ',
                $out += ' <a href="tjzy.html">查看已填报志愿</a> </p> </div>',
                new String($out)
        }),
        template('common/sxbz-include', ''),
        template('common/sys', function($data, $filename) {
            'use strict';
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                null != sysData && ($out += ' <div class="width700 tj-nopermission"> <div class="tj-icon-nopermission"></div> <h2>', $out += $escape(sysData.title), $out += '</h2> <p>', $out += $escape(sysData.content), $out += '</p> </div> '),
                $out += ' ',
                new String($out)
        }),
        ...
        template('common/tbtjzy-item', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                'wap' == agent_from ? ($out += ' <option value="">请选择', '1' == codeType ? $out += '招生单位' : '2' == codeType ? $out += '院系所' : '3' == codeType ? $out += '专业' : '4' == codeType ? $out += '研究方向' : '5' == codeType && ($out += '学习方式'), $out += '</option> ') : $out += ' <option value="">请选择</option> ',
                $out += ' ',
                $each(seachList, function(value, index) {
                    $out += ' <option value="',
                        $out += $escape(value.code),
                        $out += '" ',
                        value.code == code && ($out += 'selected="selected"'),
                        $out += '>(',
                        $out += $escape(value.code),
                        $out += ') ',
                        $out += $escape(value.name),
        ...
        template('common/tjyx-qecx', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                'web' == agent_from ? ($out += ' ', include('../web/tjyx-qecx-seach'), $out += '  <div id="content-qecxList"> ', include('../common/qecx-quto'), $out += ' </div> ', include('../common/tjyx-tips'), $out += '  ') : ($out += ' ', include('../wap/tjyx-qecx-seach'), $out += '  <div id="content-qecxList"> ', include('../common/tjyx-tips'), $out += '  </div> '),
                $out += ' <script> init_select_ss(\'',
                $out += $escape(agent_from),
                $out += '\'); //初始化区域 </script> ',
                new String($out)
        }),
        template('common/tjyx-success', '<div class="tj-nopermission"> <div class="tj-icon-success"></div> <h2>调剂意向填写已完成！</h2> <p class="tj-success-p"><a href="qecx.html">继续填写</a>&ensp;&ensp;|&ensp;&ensp;<a href="tjzy.html">查看已填写调剂意向</a></p> </div> '),
        template('common/tjyx-tbtjzy', function($data, $filename) {
            'use strict';
            var $utils = this,
        ...
        template('common/tjyx-tjzy', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' <div ',
                data.notice.status.cur_num < data.notice.status.max_num && ($out += 'class="tbtjzy-add-fixed"'),
                $out += '> ',
                flag && ($out += ' ', '' != data.vo_list.vos ? ($out += ' ', data.notice.status.cur_num < data.notice.status.max_num && ($out += ' ', 'wap' == other.agent_from && ($out += ' <div class="text-c tbtjzy-add"> <a href="qecx.html" class="tj-btn-big">新增调剂意向</a> </div> '), $out += ' '), $out += ' <div class="zy-main-div clearfix mar-t20 zy-main-postion"> ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <div class="zy-main"> ',
                        'wap' == other.agent_from && ($out += ' <div class="tjzy-top"> <div class="left-line"> ', $out += $escape($helpers.changeLetter(index)), $out += ' </div> <div class="left"> <h6>平行调剂意向</h6> <div class="timer">', $out += $escape(value.bmsjStr), $out += '</div> </div> </div> <div class="tjzy-top-line"></div> '),
                        $out += ' <div class="zy-cxt"> ',
                        'web' == other.agent_from && ($out += ' <div class="tjzy-top"> <div class="left-line"> ', $out += $escape($helpers.changeLetter(index)), $out += ' </div> <div class="left"> <h6>平行调剂意向</h6> <div class="timer">', $out += $escape(value.bmsjStr), $out += '</div> </div> </div> <div class="tjzy-top-line"></div> '),
                        $out += ' <div class="zy-cxt-dl"> <div class="clearfix"><div class="list-title">招生单位：</div><div class="list-content">(',
                        $out += $escape(value.dwdm),
                        $out += ') ',
                        $out += $escape(value.dwmc),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">院系所：</div><div class="list-content">(',
                        $out += $escape(value.yxsdm),
                        $out += ') ',
                        $out += $escape(value.yxsmc),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">专业：</div><div class="list-content">(',
                        $out += $escape(value.zydm),
                        $out += ') ',
                        $out += $escape(value.zymc),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">研究方向：</div><div class="list-content">(',
                        $out += $escape(value.yjfxdm),
                        $out += ') ',
                        $out += $escape(value.yjfxmc),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">学习方式：</div><div class="list-content">(',
                        $out += $escape(value.xxfs),
                        $out += ') ',
                        '1' == value.xxfs ? $out += '全日制' : '2' == value.xxfs && ($out += '非全日制'),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">备注：</div><div class="list-content">',
                        '' == value.bz ? $out += '无' : $out += $escape(value.bz),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">考生手机号码：</div><div class="list-content">',
                        $out += $escape(value.lxdh),
                        $out += '</div></div> <!-- <div class="clearfix list-message mar-t30"> <a href="/yjszs/tjzx/tjyxly.html?dwdm=',
                        $out += $escape(value.dwdm),
                        $out += '&yxsdm=',
                        $out += $escape(value.yxsdm),
                        $out += '&token=',
                        $out += $escape(value.zxtoken),
                        $out += '&zyId=',
                        $out += $escape(value.id),
                        $out += '&ksbh=',
                        $out += $escape(value.ksbh),
                        $out += '&lx=2" title="" class="list-message-ly" ',
                        'web' == other.agent_from && ($out += 'target="_blank"'),
                        $out += '>向招生单位留言</a> </div> --> </div> </div> <div class="tj-fixed"> ',
                        tj_usable ? ($out += ' ', '' == value.tj_id ? ($out += ' ', value.istjzyym ? $out += ' <p class="grey-status"><i class="iconfont">&#xe62a;</i>调剂志愿已达三个，不可继续转为调剂志愿</p> ' : ($out += ' <a class="tj-btn-big" href="javascript:;" onclick="template_totjzy(\'tjyx-mytjyx-post\',\'', $out += $escape(value.id), $out += '\',event)">转为调剂志愿</a> '), $out += ' ') : $out += ' <p class="grey-status"><i class="iconfont">&#xe68d;</i>已转为调剂志愿</p> ', $out += ' ') : $out += ' <p class="grey-status"><i class="iconfont">&#xe668;</i>调剂服务系统尚未开通，不可转为调剂志愿</p> ',
                        $out += ' </div> </div> '
                }), $out += ' ', data.notice.status.cur_num < data.notice.status.max_num && ($out += ' ', 'web' == other.agent_from && ($out += ' <div class="zy-main"> <a class="zy-add" href="qecx.html"> <div class="zy-add-icon">+</div> <div class="zy-add-content">填写调剂意向</div> </a> </div> '), $out += ' '), $out += ' </div> ') : ($out += ' <div class="sys-tip no-data mar-t20"> ', 'web' == other.agent_from && ($out += ' <div class="text-c"> <a href="qecx.html" class="tj-btn-big">填写调剂意向</a> </div> '), $out += ' <div class="tj-bg-zyempty"></div> <div class="tj-empty-message">您暂无调剂意向</div> </div> ', 'wap' == other.agent_from && ($out += ' <div class="text-c tbtjzy-add"> <a href="qecx.html" class="tj-btn-big">填写调剂意向</a> </div> '), $out += ' '), $out += ' '),
                $out += ' </div>',
                new String($out)
        }),
        template('common/tjyx-unuse', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $string = $utils.$string,
                $out = '';
            return $out += '<div class="width700 tj-nopermission"> <div class="tj-icon-nopermission"></div> <h2>无权使用</h2> ',
                '' == page_error ? $out += ' <p>本调剂系统适用考生范围为参加网上报名，第一志愿没有被招生单位录取并且达到全国硕士研究生招生考试考生进入复试的初试成绩基本要求的统考、管理类联考、法硕联考、少数民族骨干计划以及网上报名时选择退役大学生士兵计划考生。</p> ' : ($out += ' <p class="tj-success-p">', $out += $string(page_error), $out += '</p> '),
                $out += ' </div> ',
                new String($out)
        }),
        template('common/tjzy', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $escape = ($data.value, $data.index, $utils.$escape),
                $out = '';
            return $out += ' <div class="info-prompt mar-t20"> <ul> <li>参加调剂的考生可一次填报三个平行调剂志愿，提交后的调剂志愿在招生单位设置的时间内（不超过36小时）不允许修改（每个志愿单独计时），以供招生单位下载志愿信息和决定是否通知考生参加复试。锁定时间到达后，考生可继续填报其他志愿。</li> </ul> </div> <div ',
                data.notice.status.cur_num < data.notice.status.max_num && ($out += 'class="tbtjzy-add-fixed"'),
                $out += '> ',
                flag && ($out += ' ', '' != data.vo_list.vos ? ($out += ' ', data.notice.status.cur_num < data.notice.status.max_num && ($out += ' ', 'wap' == other.agent_from && ($out += ' <div class="text-c tbtjzy-add"> <a href="qecx.html" class="tj-btn-big">新增调剂志愿</a> </div> '), $out += ' '), $out += ' <div class="zy-main-div clearfix mar-t20 zy-main-postion"> ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <div class="zy-main"> ',
                        'wap' == other.agent_from && ($out += ' <div class="tjzy-top"> <div class="left-line"> ', $out += $escape($helpers.changeLetter(index)), $out += ' </div> <div class="left"> <h6>平行调剂志愿</h6> <div class="timer">', $out += $escape(value.bmsjStr), $out += '</div> </div> </div> <div class="tjzy-top-line"></div> '),
                        $out += ' <div class="zy-cxt"> ',
                        'web' == other.agent_from && ($out += ' <div class="tjzy-top"> <div class="left-line"> ', $out += $escape($helpers.changeLetter(index)), $out += ' </div> <div class="left"> <h6>平行调剂志愿</h6> <div class="timer">', $out += $escape(value.bmsjStr), $out += '</div> </div> </div> <div class="tjzy-top-line"></div> '),
                        $out += ' <div class="zy-cxt-dl ',
                        'web' == other.agent_from && ($out += 'mar-t10'),
                        $out += '"> <div class="clearfix"><div class="list-title">招生单位：</div><div class="list-content">(',
                        $out += $escape(value.dwdm),
                        $out += ') ',
                        $out += $escape(value.dwmc),
                        $out += '</div></div> <div class="clearfix"><div class="list-title">院系所：</div><div class="list-content">(',
        ...
        template('wap/qecx-list-item', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                '' == data.vo_list.vos ? $out += ' <li class="tj-list-item tj-list-item-noresult">该条件下没有查询到计划余额信息</li> ' : ($out += ' ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <li class="tj-list-item"> <table id="qe_',
                        $out += $escape(value.id),
                        $out += '"> <tr> <td colspan="2" class="tj-list-item-dw"><a href="/sch/tjzc--method-listTjPub,yxdm-',
                        $out += $escape(value.dwdm),
                        $out += '.dhtml" target="_blank">(',
                        $out += $escape(value.dwdm),
                        $out += ') ',
                        $out += $escape(value.dwmc),
                        $out += '</a></td> <td rowspan="5" class="tj-list-item-sq"> ',
                        '' == value.sfmzjybyq && ($out += ' <span class="tj-tip tsyq-has-title tj-sqtj-text" onclick="template_zytb_tj(\'', $out += $escape(value.id), $out += '\')">申请条件</span> '),
                        $out += ' ',
                        value.hasit ? $out += '  <button class="tj-sqtj-btn" style="background: #ccc;">已申请</button> ' : ($out += ' ', '' == value.sfmzjybyq ? ($out += ' <button class="tj-sqtj-btn" ', '' == value.sfmzyq ? ($out += 'onclick="template_zytb_add(\'', $out += $escape(value.id), $out += '\')"') : ($out += 'onclick="layer.alert(\'', $out += $escape(value.sfmzyq), $out += '\',{title: \'提示\'})"'), $out += '>申请</button> ') : ($out += ' <a class="action-text" href="https://yz.chsi.com.cn/kyzx/tjzd/202203/20220311/2172264654.html">', $out += $escape(value.sfmzjybyq), $out += '</a> '), $out += ' '),
                        $out += ' <span class="tj-qe">计划余额：<br><span class="tj-qe-y">',
                        0 != value.qers ? ($out += $escape(value.qers), $out += '人') : $out += '有',
                        $out += '</span></span> </td> </tr> <tr> <td class="tj-list-item-title">院系所：</td> <td class="tj-list-item-content yxxx">(',
                        $out += $escape(value.yxsdm),
                        $out += ')',
                        $out += $escape(value.yxsmc),
                        $out += '</td> </tr> <tr> <td class="tj-list-item-title">专业：</td> <td class="tj-list-item-content zyxx">(',
                        $out += $escape(value.zydm),
                        $out += ')',
                        $out += $escape(value.zymc),
                        $out += '</td> </tr> <tr> <td class="tj-list-item-title">研究方向：</td> <td class="tj-list-item-content zyxx">(',
                        $out += $escape(value.yjfxdm),
                        $out += ') ',
                        $out += $escape(value.yjfxmc),
                        $out += '</td> </tr> <tr> <td class="tj-list-item-title">学习方式：</td> <td class="tj-list-item-content xxfs">',
                        '1' == value.xxfs ? $out += '全日制' : '2' == value.xxfs && ($out += '非全日制'),
                        $out += '</td> </tr> <tr> <td colspan="3"> <div class="tj-list-item-tjsm"><span style="color: #f57716">调剂说明：</span>',
                        '' == value.bz ? $out += '招生单位对该专业暂无特殊要求' : $out += $escape(value.bz),
                        $out += '</div> <div class="tj-list-item-time"><span style="color: #f57716">最后更新：</span>',
                        $out += $escape($helpers.gxsjHelper(value.gxsj)),
                        $out += '</div> </td> </tr> <!-- <tr> <td colspan="2" class="tj-list-tsyq"> <span class="tsyq tsyq-has"> <i class="iconfont">&#xe652;</i> </span> <span class="time"><i class="iconfont">&#xe659;</i>',
                        $out += $escape(value.fbsjStr),
                        $out += '</span> </td> </tr> --> </table> </li> '
                }), $out += ' '),
                $out += ' ',
                new String($out)
        }),
        template('wap/qecx-list', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += '<div id="content-qecxList">  <div class="info-prompt mar-t20"> <ul> <li>提醒：招生单位发布的调剂计划余额每次更新后将至少持续12小时，请各位考生结合各招生单位设定的志愿锁定时间综合考虑，合理申请，错峰报名。</li> </ul> </div> ',
                '' != data.vo_list.vos && ($out += '  <div class="tj-seach-sort clearfix"> <input type="hidden" id="sysname" value="qecx-list"> <ul> <li class="sort-icon" data-name="dwmc" data-value="">单位<span class="sort-icon-i"></span></li> <li class="sort-icon" data-name="yxsmc" data-value="">院系<span class="sort-icon-i"></span></li> <li class="sort-icon" data-name="zymc" data-value="">专业<span class="sort-icon-i"></span></li> <li class="sort-icon" data-name="qers" data-value="">人数<span class="sort-icon-i"></span></li> <li class="sort-icon sort-icon-more" data-name="xxfs" data-value="">学习方式<span class="sort-icon-i"></span></li> </ul> </div> '),
                $out += ' <div class="tj-list" id="wrapper"> <ul> ',
                include('../wap/qecx-list-item'),
                $out += ' </ul> </div> ',
                include('../common/tips'),
                $out += '  </div> ',
                new String($out)
        ...
        template('wap/qecx-seach', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' <div class="tj-seach-conditions mar-t10" id="tj_seach_change"> <div class="conditions-warp clearfix"> <div class="conditions-tabs"> <ul> <li ',
                'accurate' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'\',\'accurate\',event)">精确查询</li> <li ',
                'fuzzy' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'\',\'fuzzy\',event)">模糊查询</li> </ul> </div> <div class="conditions_show"> <input type="hidden" id="sysname" value="qecx-list-wap"> <form id="tj_seach_form"> <input type="hidden" name="pageSize" id="page_size"> <input type="hidden" name="start" id="start"> <input type="hidden" name="orderBy" id="order_by"> ',
                $out += 'fuzzy' == seachType ? ' <input type="text" name="mhcx" value="1" style="display: none;"> <select id="ss" name="ssdm2"> <option value="">（省市）不限</option> </select> <select id="xxfs" name="xxfs2"> <option value="">（学习方式）不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> <input type="text" placeholder="请输入招生单位或专业名称的关键词" name="dwmc2" id="dwxx" maxlength="100" autocomplete="off" /> <a class="tj-btn-middle conditions-btn tj-seach-btn" href="javascript:;" onclick="template_qecx_seach(\'qecx-list-wap\',this,event,\'0\')">查 询</a> ' : ' <select id="ss" name="ssdm"> <option value="">（省市）不限</option> </select> <input type="text" placeholder="请输入完整的单位名称" name="dwmc" id="dwxx" maxlength="100" autocomplete="off" /> <select id="xxfs" name="xxfs"> <option value="">（学习方式）不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> <input type="text" placeholder="请输入完整的专业名称" name="zymc" id="zyxx" maxlength="100" autocomplete="off" /> <div class="qers-content"> <input type="text" placeholder="请输入计划余额人数" name="qers" id="qexx" maxlength="4" onkeyup="this.value=this.value.replace(/[^\\d]/g,\'\')" autocomplete="off" /><span>人以上</span> </div> <a class="tj-btn-middle conditions-btn tj-seach-btn" href="javascript:;" onclick="template_qecx_seach(\'qecx-list-wap\',this,event,\'1\')">查 询</a> ',
                $out += ' </form> </div> </div> ',
                'fuzzy' != seachType && ($out += ' <script> $(function () { fuzzy_query("dwxx-init","tj"); fuzzy_query("zyxx-init","tj"); }); </script> '),
                $out += ' </div>',
                new String($out)
        }),
        template('wap/tjyx-qecx-list-item', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                '' == data.vo_list.vos ? $out += ' <li class="tj-list-item tj-list-item-noresult">该条件下没有查询到余额信息</li> ' : ($out += ' ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <li class="tj-list-item"> <table id="qe_',
                        $out += $escape(value.id),
                        $out += '"> <tr> <td colspan="2" class="tj-list-item-dw"><a href="/sch/tjzc--method-listTjPub,yxdm-',
                        $out += $escape(value.dwdm),
                        ...
                        $out += ') ',
                        $out += $escape(value.dwmc),
                        $out += '</a></td> <td rowspan="5" class="tj-list-item-sq"> <!-- <span class="tj-tip tsyq-has-title" onclick="template_zytb_tj(\'',
                        $out += $escape(value.id),
                        $out += '\')">申请条件</span> --> ',
                        ...
                        $out += ' <!-- <span class="tj-qe"> 余额',
                        0 != value.qers ? ($out += $escape(value.qers), $out += '人') : $out += '有',
                        $out += '</span> --> </td> </tr> <tr> <td class="tj-list-item-title">院系所：</td> <td class="tj-list-item-content yxxx">(',
                        $out += $escape(value.yxsdm),
                        $out += ')',
                        $out += $escape(value.yxsmc),
                        $out += '</td> </tr> <tr> <td class="tj-list-item-title">专业：</td> <td class="tj-list-item-content zyxx">(',
                        ...
                        $out += '</td> </tr> <tr> <td class="tj-list-item-title">学习方式：</td> <td class="tj-list-item-content xxfs">',
                        '1' == value.xxfs ? $out += '全日制' : '2' == value.xxfs && ($out += '非全日制'),
                        $out += '</td> </tr> <tr> <td colspan="3"> <div class="tj-list-item-tjsm"><span style="color: #f57716">特殊说明：</span>',
                        '' == value.bz ? $out += '招生单位对该专业暂无特殊要求' : $out += $escape(value.bz),
                        $out += '</div> <div class="tj-list-item-time">',
                        $out += $escape(value.fbsjStr),
                        $out += '</div> </td> </tr> </table> </li> '
                }), $out += ' '),
                $out += ' ',
                new String($out)
        }),
        template('wap/tjyx-qecx-list', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += '<div id="content-qecxList"> ',
                '' != data.vo_list.vos && ($out += '  <div class="tj-seach-sort clearfix"> <input type="hidden" id="sysname" value="tjyx-qecx-list"> <ul> <li class="sort-icon" data-name="dwmc" data-value="">单位<span class="sort-icon-i"></span></li> <li class="sort-icon" data-name="yxsmc" data-value="">院系<span class="sort-icon-i"></span></li> <li class="sort-icon" data-name="zymc" data-value="">专业<span class="sort-icon-i"></span></li> <li class="sort-icon sort-icon-more" data-name="xxfs" data-value="">学习方式<span class="sort-icon-i"></span></li> <li class="sort-icon sort-icon-more" data-name="fbsjStr" data-value="">发布时间<span class="sort-icon-i"></span></li> </ul> </div> '),
                $out += ' <div class="tj-list" id="wrapper"> <ul> ',
                include('../wap/tjyx-qecx-list-item'),
                $out += ' </ul> </div> ',
                include('../common/tjyx-tips'),
                $out += '  </div> ',
                new String($out)
        }),
        template('wap/tjyx-qecx-seach', function($data, $filename) {
            ...
            return $out += ' <div class="tj-seach-conditions mar-t10" id="tj_seach_change"> <div class="conditions-warp clearfix"> <div class="conditions-tabs"> <ul> <li ',
                'accurate' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'tjyx\',\'accurate\',event)">精确查询</li> <li ',
                'fuzzy' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'tjyx\',\'fuzzy\',event)">模糊查询</li> </ul> </div> <div class="conditions_show"> <input type="hidden" id="sysname" value="tjyx-qecx-list-wap"> <form id="tj_seach_form"> <input type="hidden" name="pageSize" id="page_size"> <input type="hidden" name="start" id="start"> <input type="hidden" name="orderBy" id="order_by"> ',
                $out += 'fuzzy' == seachType ? ' <input type="text" name="mhcx" value="1" style="display: none;"> <select id="ss" name="ssdm2"> <option value="">（省市）不限</option> </select> <select id="xxfs" name="xxfs2"> <option value="">（学习方式）不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> <input type="text" placeholder="请输入招生单位或专业名称的关键词" name="dwmc2" id="dwxx" maxlength="100" autocomplete="off" /> <a class="tj-btn-middle conditions-btn tj-seach-btn" href="javascript:;" onclick="template_qecx_seach(\'tjyx-qecx-list-wap\',this,event,\'0\')">查 询</a> ' : ' <select id="ss" name="ssdm"> <option value="">（省市）不限</option> </select> <input type="text" placeholder="请输入完整的单位名称" name="dwmc" id="dwxx" maxlength="100" autocomplete="off" /> <select id="xxfs" name="xxfs"> <option value="">（学习方式）不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> <input type="text" placeholder="请输入完整的专业名称" name="zymc" id="zyxx" maxlength="100"/> <a class="tj-btn-middle conditions-btn tj-seach-btn" href="javascript:;" onclick="template_qecx_seach(\'tjyx-qecx-list-wap\',this,event,\'1\')">查 询</a> ',
                $out += ' </form> </div> </div> ',
                'fuzzy' != seachType && ($out += ' <script> $(function () { fuzzy_query("tjyx-dwxx-init","tjyx"); fuzzy_query("tjyx-zyxx-init","tjyx"); }); </script> '),
                $out += ' </div>',
                new String($out)
        }),
        template('web/page', function($data, $filename) {
            ...
            var $utils = this,
                ...
                $out = '';
            return $out += ' ',
                data.vo_list.pagenation && ($out += ' ', data.vo_list.pagenation.totalCount > 0 && ($out += ' <div class="tj-paging mar-t30 clearfix"> <ul> ', $out += $string($helpers.pagerander(data.vo_list.pagenation)), $out += ' </ul> </div> '), $out += ' '),
                $out += ' ',
                new String($out)
        }),
        template('web/qecx-list', function($data, $filename) {
            ...
            return $out += ' <div class="mar-t10" id="content-qecxList"> <div class="info-prompt mar-t10"> <ul> <li>提醒：招生单位发布的调剂计划余额每次更新后将至少持续12小时，请各位考生结合各招生单位设定的志愿锁定时间综合考虑，合理申请，错峰报名。</li> </ul> </div> <input type="hidden" id="sysname" value="qecx-list"> <table class="tj-table"> <thead> <tr> <th width="190"><span class="sort-icon" data-name="dwmc" data-value="">招生单位</span></th> <th width="175"><span class="sort-icon" data-name="yxsmc" data-value="">院系所</span></th> <th width="175"><span class="sort-icon" data-name="zymc" data-value="">专业</span></th> <th><span class="sort-icon" data-name="yjfxmc" data-value="">研究方向</span></th> <th width="85"><span class="sort-icon" data-name="xxfs" data-value="">学习方式</span></th> <th width="120"><span class="sort-icon" data-name="qers" data-value="">计划余额</span></th> <th width="120">最后更新</span></th> <th width="70">申请条件</th> <th width="120">操作</th> </tr> </thead> <tbody> ',
                flag && ($out += ' ', data.vo_list.pagenation ? ($out += ' ', data.vo_list.pagenation.totalCount > 0 ? ($out += ' ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <tr> <td><a href="/sch/tjzc--method-listTjPub,yxdm-',
                        $out += $escape(value.dwdm),
                        $out += '.dhtml" target="_blank">(',
                        $out += $escape(value.dwdm),
                        $out += ') ',
            ...
            return $out += ' ',
                flag && ($out += ' <div class="mar-t10" id="content-qecxList"> <input type="hidden" id="sysname" value="tjyx-qecx-list"> <table class="tj-table"> <thead> <tr> <th width="200"><span class="sort-icon" data-name="dwmc" data-value="">招生单位</span></th> <th width="200"><span class="sort-icon" data-name="yxsmc" data-value="">院系所</span></th> <th width="200"><span class="sort-icon" data-name="zymc" data-value="">专业</span></th> <th><span class="sort-icon" data-name="yjfxmc" data-value="">研究方向</span></th> <th width="90"><span class="sort-icon" data-name="xxfs" data-value="">学习方式</span></th> <th width="145"><span class="sort-icon" data-name="fbsjStr" data-value="">发布时间</span></th> <th width="75">特殊说明</th> <th width="120">操作</th> </tr> </thead> <tbody> ', flag && ($out += ' ', data.vo_list.pagenation ? ($out += ' ', data.vo_list.pagenation.totalCount > 0 ? ($out += ' ', $each(data.vo_list.vos, function(value, index) {
                    $out += ' <tr> <td><a href="/sch/tjzc--method-listTjPub,yxdm-',
                        ...
                        $out += $escape(value.fbsjStr),
                        $out += '</td> <td class="text-c"> ',
                        '' != value.bz ? ($out += ' <span data-tip="', $out += $escape(value.bz), $out += '" class="tj-tip color-blue tjyx-tip">查看</span> ') : $out += ' <span data-tip="招生单位对该专业暂无特殊要求" class="tj-tip color-blue tjyx-tip">查看</span> ',
                        $out += ' </td> <td class="text-c"> ',
                        value.hasit ? $out += ' 已填写 ' : ($out += ' ', '' == value.sfmzjybyq ? ($out += ' <a href="javascript:;" class="tj-btn-small" onclick="template_zytb_add(\'', $out += $escape(value.id), $out += '\')">填写</a> ') : ($out += ' <a data-tip="具体请参见教育部调剂政策" class="tj-tip tjyx-tip action-text" target="_blank" href="https://yz.chsi.com.cn/kyzx/tjzd/202203/20220311/2172264654.html">', $out += $escape(value.sfmzjybyq), $out += '</a> '), $out += ' '),
                        $out += ' </td> </tr> '
                }), $out += ' ') : $out += ' <tr> <td colspan="8">该条件下没有查询到调剂意向余额信息</td> </tr> ', $out += ' ') : $out += ' <tr> <td colspan="8">该条件下没有查询到调剂意向余额信息</td> </tr> ', $out += ' '), $out += ' </tbody> </table> ', function(filename, data) {
                    data = data || $data;
                    var text = $utils.$include(filename, data, $filename);
                    $out += text
                }('../web/page'), $out += ' </div> '),
        ...
        template('web/tjyx-qecx-seach', function($data, $filename) {
            ...
            var $utils = this,
                seachType = ($utils.$helpers, $data.seachType),
                $out = '';
            return $out += ' <div class="qecx-tip mar-t20" id="tj_seach_change"> <h2>查询调剂意向余额信息</h2> <div class="tj-seach"> <div class="tj-seach-tabs"> <ul> <li ',
                'accurate' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'tjyx\',\'accurate\',event)">精确查询</li> <li ',
                'fuzzy' == seachType && ($out += 'class="tabs-check"'),
                $out += ' onclick="change_seach(\'tjyx\',\'fuzzy\',event)">模糊查询</li> </ul> </div> <form id="tj_seach_form"> <input type="hidden" name="pageSize" id="page_size"> <input type="hidden" name="start" id="start"> <input type="hidden" name="orderBy" id="order_by"> ',
                $out += 'fuzzy' == seachType ? ' <input type="text" name="mhcx" value="1" style="display: none;"> <table> <tr> <td width="80" class="text-r">所在省市：</td> <td> <select name="ssdm2" id="ss" class="tj-select width112"> <option value="">请选择省市</option> </select> </td> <td width="80" class="text-r">学习方式：</td> <td> <select name="xxfs2" id="xxfs" class="tj-select width94"> <option value="">不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> </td> <td> <input type="text" name="dwmc2" id="dwxx" value="" class="tj-input width240" maxlength="100" placeholder="请输入招生单位或专业名称的关键词" autocomplete="off" /> </td> <td> <a href="javascript:;" class="tj-btn-middle tj-seach-btn" onclick="template_qecx_seach(\'tjyx-qecx-list\',this,event,\'0\')">查 询</a> </td> <td width="420"></td> </tr> </table> ' : ' <table> <tr> <td width="80" class="text-r">所在省市：</td> <td> <select name="ssdm" id="ss" class="tj-select width112"> <option value="">请选择省市</option> </select> </td> <td width="80" class="text-r">招生单位：</td> <td> <input type="text" name="dwmc" id="dwxx" value="" class="tj-input width145" maxlength="100" placeholder="请输入完整的单位名称"/> </td> <td width="80" class="text-r">学习方式：</td> <td> <select name="xxfs" id="xxfs" class="tj-select width94"> <option value="">不限</option> <option value="1">全日制</option> <option value="2">非全日制</option> </select> </td> <td width="50" class="text-r">专业：</td> <td> <input type="text" name="zymc" id="zyxx" value="" class="tj-input width145" maxlength="100" placeholder="请输入完整的专业名称"/> </td> <td width="100" class="text-c"> <a href="javascript:;" class="tj-btn-middle tj-seach-btn" onclick="template_qecx_seach(\'tjyx-qecx-list\',this,event,\'1\')">查 询</a> </td> <td colspan="3" width="280">  </td> </tr> </table> ',
                $out += ' </form> </div> </div> ',
                $out += 'fuzzy' != seachType ? ' <script> $(function () { fuzzy_query("tjyx-dwxx-init","tjyx"); fuzzy_query("tjyx-zyxx-init","tjyx"); JPlaceHolder.init(); }); </script> ' : ' <script> $(function () { JPlaceHolder.init(); }); </script> ',
                new String($out)
        })
}();
var Mainfest = {
    'dwxx-init': {
        template: '',
        interface_url: 'https://t1.chei.com.cn/sytj/stu/getcachezsdws/',
        description: '单位搜索'
    },
    'zyxx-init': {
        template: '',
        interface_url: 'https://t1.chei.com.cn/sytj/stu/getcachezys/',
        description: '专业搜索'
    },
    'tjyx-dwxx-init': {
        template: '',
        interface_url: '/sytj/stu/getcachezsdwstjyx/',
        description: '单位搜索-调剂意向'
    },
    'tjyx-zyxx-init': {
        template: '',
        interface_url: '/sytj/stu/getcachezystjyx/',
        description: '专业搜索-调剂意向'
    },
    'notice-init': {
        template: 'common/notice',
        interface_url: '/sytj/stu/getnotice.action',
        description: '获取提醒信息'
    },
    'tjyx-index': {
        template: 'common/tjyx-index',
        interface_url: '',
        description: '调剂意向-首页初始化'
    },
    'tjyx-qecx': {
        template: 'common/tjyx-qecx',
        interface_url: '',
        description: '调剂意向-缺额查询初始化'
    },
    'tjyx-qecx-seach': {
        template: 'web/tjyx-qecx-seach',
        interface_url: '/sytj/stu/getzsdws.action',
        description: '调剂意向-缺额查询-获取招生单位'
    },
    'tjyx-qecx-seach-wap': {
        template: 'wap/tjyx-qecx-seach',
        interface_url: '/sytj/stu/getzsdws.action',
        description: '调剂意向-缺额查询-获取招生单位-移动端'
    },
    'tjyx-qecx-list': {
        template: 'web/tjyx-qecx-list',
        interface_url: '/sytj/stu/tjyxqexxcx.action',
        description: '调剂意向-缺额查询-列表'
    },
    'tjyx-qecx-list-wap': {
        template: 'wap/tjyx-qecx-list',
        interface_url: '/sytj/stu/tjyxqexxcx.action',
        description: '调剂-缺额查询-列表-移动端'
    },
    'tjyx-mytjyx': {
        template: 'common/tjyx-tjzy',
        interface_url: '/sytj/stu/gettjyx.action',
        description: '调剂意向-我的意向'
    },
    'tjyx-mytjyx-post': {
        template: 'common/tjyx-tjzy',
        interface_url: '/sytj/stu/tjyxtotjapply.action',
        description: '调剂意向-我的意向-转为调剂志愿'
    },
    index: {
        template: 'common/index',
        interface_url: '',
        description: '调剂-首页初始化'
    },
    cjxx: {
        template: 'common/cjxx',
        interface_url: '/sytj/stu/sytjcjdata.action',
        description: '调剂-成绩信息'
    },
    'tjyx-cjxx': {
        template: 'common/cjxx',
        interface_url: '/sytj/stu/tjyxcjdata.action',
        description: '调剂-成绩信息'
    },
    mytjzy: {
        template: 'common/tjzy',
        interface_url: '/sytj/stu/gettjapply.action',
        description: '调剂-我的志愿'
    },
    qecx: {
        template: 'common/qecx',
        interface_url: '',
        description: '调剂-缺额查询'
    },
    'qecx-seach': {
        template: 'web/qecx-seach',
        interface_url: '/sytj/stu/getzsdws.action',
        description: '调剂/意向-缺额查询-获取招生单位'
    },
    'qecx-seach-wap': {
        template: 'wap/qecx-seach',
        interface_url: '/sytj/stu/getzsdws.action',
        description: '调剂/意向-缺额查询-获取招生单位-移动端'
    },
    'qecx-list': {
        template: 'web/qecx-list',
        interface_url: '/sytj/stu/sytjqexxcx.action',
        description: '调剂-缺额查询-列表'
    },
    'qecx-list-wap': {
        template: 'wap/qecx-list',
        interface_url: '/sytj/stu/sytjqexxcx.action',
        description: '调剂-缺额查询-列表-移动端'
    },
    tbtjzy: {
        template: 'common/tbtjzy',
        interface_url: '/sytj/stu/gettjqexx.action',
        description: '调剂-缺额查询-志愿填报'
    },
    'tbtjzy-edit': {
        template: 'common/tbtjzy-edit',
        interface_url: '/sytj/stu/gettjapplyone.action',
        description: '调剂-缺额查询-志愿修改'
    },
    'tbtjzy-edit-post': {
        template: 'common/tbtjzy-edit',
        interface_url: '/sytj/stu/modtjapply.action',
        description: '调剂-缺额查询-志愿修改-提交'
    },
    'tbtjzy-ssdm': {
        template: 'common/tbtjzy-item',
        interface_url: '',
        description: '调剂-缺额查询-志愿填报-省市级联'
    },
    'tbtjzy-dwxx': {
        template: 'common/tbtjzy-item',
        interface_url: '/sytj/stu/getqezsdws.action',
        description: '调剂-缺额查询-志愿填报-单位级联'
    },
    'tbtjzy-yxxx': {
        template: 'common/tbtjzy-item',
        interface_url: '/sytj/stu/getqeyxss.action',
        description: '调剂-缺额查询-志愿填报-院系所级联'
    },
    'tbtjzy-zyxx': {
        template: 'common/tbtjzy-item',
        interface_url: '/sytj/stu/getqezys.action',
        description: '调剂-缺额查询-志愿填报-专业级联'
    },
    'tbtjzy-fxxxx': {
        template: 'common/tbtjzy-item',
        interface_url: '/sytj/stu/getqeyjfxs.action',
        description: '调剂-缺额查询-志愿填报-研究方向级联'
    },
    'tbtjzy-xffs': {
        template: 'common/tbtjzy-item',
        interface_url: '/sytj/stu/getqexxfss.action',
        description: '调剂-缺额查询-志愿填报-学习方式'
    },
    'tjyx-tbtjzy': {
        template: 'common/tjyx-tbtjzy',
        interface_url: '/sytj/stu/gettjyxqexx.action',
        description: '调剂意向-缺额查询-志愿填报'
    },
    'tbtjzy-post': {
        template: 'common/tbtjzy',
        interface_url: '/sytj/stu/addtjapply.action',
        description: '调剂-缺额查询-提交志愿填报'
    },
    'tjyx-tbtjzy-post': {
        template: 'common/tjyx-tbtjzy',
        interface_url: '/sytj/stu/addtjyx.action',
        description: '调剂-缺额查询-提交志愿填报'
    },
    fstz: {
        template: 'common/fstz',
        interface_url: '/sytj/stu/gettjfstz.action',
        description: '调剂-我的复试通知'
    },
    'fstz-post-accept': {
        template: 'common/fstz',
        interface_url: '/sytj/stu/agreefstz.action',
        description: '调剂-我的复试通知-提交-接受'
    },
    'fstz-post-reject': {
        template: 'common/fstz',
        interface_url: '/sytj/stu/disagreefstz.action',
        description: '调剂-我的复试通知-提交-拒绝'
    },
    dlqtz: {
        template: 'common/dlqtz',
        interface_url: '/sytj/stu/gettjdlqtz.action',
        description: '调剂-我的待录取通知'
    },
    'dlqtz-post-accept': {
        template: 'common/dlqtz',
        interface_url: '/sytj/stu/agreedlqtz.action',
        description: '调剂-我的待录取通知-提交'
    },
    'dlqtz-post-reject': {
        template: 'common/dlqtz',
        interface_url: '/sytj/stu/disagreedlqtz.action',
        description: '调剂-我的待录取通知-提交'
    },
    'dlqtz-post-acceptcancel': {
        template: 'common/dlqtz',
        interface_url: '/sytj/stu/agreecanceldlqtz.action',
        description: '调剂-我的待录取通知-提交'
    },
    'dlqtz-post-rejectcancel': {
        template: 'common/dlqtz',
        interface_url: '/sytj/stu/disagreecanceldlqtz.action',
        description: '调剂-我的待录取通知-提交'
    },
    success: {
        template: 'common/success',
        interface_url: '',
        description: '调剂-志愿填报-成功'
    },
    'tjyx-success': {
        template: 'common/tjyx-success',
        interface_url: '',
        description: '调剂-志愿填报-成功'
    },
    unuse: {
        template: 'common/unuse',
        interface_url: '/sytj/tj/sytjshouyedata.action',
        description: '调剂-志愿填报-权限'
    },
    'tjyx-unuse': {
        template: 'common/tjyx-unuse',
        interface_url: '/sytj/tjyx/sytjshouyedata.action',
        description: '调剂意向-志愿填报-权限'
    },
    'bdzkz-post': {
        template: '',
        interface_url: '/sytj/stu/bduserandksbh.action',
        description: '绑定准考证号'
    },
    'tj-tjjcap': {
        template: 'common/tjjcap',
        interface_url: '/sytj/stu/getgg.action',
        description: '调剂-调剂进程安排'
    },
    'tj-sqtj': {
        template: 'common/sqtj',
        interface_url: '/sytj/stu/gettjbktj.action',
        description: '调剂-申请条件'
    },
    yzytz: {
        template: 'common/yzytz',
        interface_url: '/sytj/stu/getyzytz.action',
        description: '调剂-一志愿通知'
    },
    'yzy-dlqtz-accept': {
        template: 'common/yzytz',
        interface_url: '/sytj/stu/agreeyzydlqtz.action',
        description: '调剂-一志愿待录取通知-提交-接受'
    },
    'yzy-dlqtz-reject': {
        template: 'common/yzytz',
        interface_url: '/sytj/stu/disagreeyzydlqtz.action',
        description: '调剂-一志愿待录取通知-提交-拒绝'
    },
    'yzy-dlqtz-cancel-accept': {
        template: 'common/yzytz',
        interface_url: '/sytj/stu/agreecancelyzydlqtz.action',
        description: '调剂-一志愿待录取通知-取消-接受'
    }
};
jQuery.cookie = function(name, value, options) {
        ...
        options = options || {},
            null === value && (value = '', options = $.extend({}, options), options.expires = -1);
        var expires = '';
        if (options.expires && ('number' == typeof options.expires || options.expires.toUTCString)) {
            var date;
            'number' == typeof options.expires ? (date = new Date, date.setTime(date.getTime() + 24 * options.expires * 60 * 60 * 1000)) : date = options.expires,
    ...
    AjaxPolling.prototype = {
            ...
            $.ajax({
                ...
                error: function(xhr, textstatus, errothown) {
                    if (parseInt(xhr.status, 10) >= 500) {
                        if (p.retryCount++, p.onError(xhr, p.retryCount), p.retryCount <= p.retryLimit) {
                            var _this = this;
                            return void(p.setTimeout = window.setTimeout(function() {
                                $.ajax(_this)
                ...
                success: function(data) {
                    if (++p.pollingCount < p.pollingLimit) {
                        if (clearTimeout(p.setTimeout), p.everyPolling(data), 'success' == data.state && (p.pollingCount = 0, p.onSuccess(data)), 'wait' == data.state || 'update' == data.state) {
                            var _this = this;
                            return void(p.setTimeout = window.setTimeout(function() {
                                $.ajax(_this)
                            }, p.pollingDelay + p.delayGaps * p.pollingCount))
                        }
                        return 'cancel' == data.state ? p.onCancel && p.onCancel(data) : 'exception' == data.state ? p.onException && p.onException(data) : void console.log('error:服务没有正确返回结果')
                    }
                    p.onTimeOut()
                }
            })
        ...
        start: function() {
            this.useProgressBar && this.progressBar(this.taskId),
                this.callServer()
        },
        progressBar: function(taskId) {
            $('#' + taskId).addClass('progress').html('<div class="progress active"><div class="progress-bar ch-progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div></div>')
        },
        changeProgressBar: function(data) {
            var progressWidth = function(n) {
                return (Math.round(10000 * n) / 100).toFixed(0) + '%'
            }(data.percent / 100);
...
var MyValidator = function() {
    var handleSubmit = function() {
        $('.fromSubmit').validate({
                ...
                messages: {
                    ssdm: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择省市'
                    },
                    dwxx: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择招生单位'
                    },
                    yxxx: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择院系所'
                    },
                    zyxx: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择专业'
                    },
                    fxxxx: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择研究方向'
                    },
                    xxfs: {
                        required: '<i class="iconfont">&#xe67c;</i>请选择学习方式'
                    },
                    yddh: {
                        required: '<i class="iconfont">&#xe67c;</i>请填写手机联系方式',
                        maxlength: '<i class="iconfont">&#xe67c;</i>手机号长度不能超过50字符！'
                    },
                    bz: {
                        maxlength: '<i class="iconfont">&#xe67c;</i>备注长度不能超过50字符！'
                    },
                    ksbh: {
                        required: '<i class="iconfont">&#xe67c;</i>请填写准考证号',
                        maxlength: '<i class="iconfont">&#xe67c;</i>准考证号长度不能超过15字符！'
                    }
                },
                highlight: function(element) {},
                success: function(label) {},
                errorPlacement: function(error, element) {
                    element.siblings('.tj-error').length > 0 ? element.siblings('.tj-error').html(error) : $('.tj-error').html(error)
                },
                submitHandler: function(form) {
                    form.submit()
                }
            }),
            $('.fromSubmit input').keypress(function(e) {
                if (13 == e.which) return $('.fromSubmit').validate().form() && $('.fromSubmit').submit(), !1
            })
    };
    return {
        init: function() {
...
$(function() {
    $('body').on('click', '.wap-menu', function() {
            $('.wap-menu-list').show(),
                $('.menu-list-warp-bg').show(),
                $('body').addClass('van-overflow-hidden')
        }),
        $('body').on('click', '.iconfont-close', function() {
            $('.wap-menu-list').hide(),
                $('.menu-list-warp-bg').hide(),
                $('body').removeClass('van-overflow-hidden')
        }),
        $('body').on('click', '.wap-expend', function() {
            $('#wap-expend-warp').hasClass('expend') ? ($('#wap-expend-warp').removeClass('expend'), $(this).removeClass('expend')) : ($('#wap-expend-warp').addClass('expend'), $(this).addClass('expend'))
        }),
        $('body').on('mouseover', '.tjyx-tip', function() {
            var _tipMessage = html2Escape($(this).attr('data-tip'));
            if ('' != _tipMessage) {
                var index = layer.tips(_tipMessage, this, {
                    tips: [
                        1,
                        '#e5db97'
                    ],
        ...
        $('body').on('mouseover', '.ewm-fix', function() {
            $('.ewm-detail').removeClass('tj-hide'),
                $(this).on('mouseout', function() {
                    $('.ewm-detail').addClass('tj-hide')
                })
        }),
        $('body').on('touchend', '.ewm-wap', function() {
            $('.wap-overlay').removeClass('tj-hide'),
                $('.ewm-detail-wap').removeClass('tj-hide')
        }),
        $('body').on('touchend', '.wap-overlay', function() {
            setTimeout(function() {
                $('.wap-overlay').addClass('tj-hide'),
                    $('.ewm-detail-wap').addClass('tj-hide')
            }, 100)
        }),
        $('body').on('touchend', '.ewm-detail-wap .close', function() {
            setTimeout(function() {
                $('.wap-overlay').addClass('tj-hide'),
                    $('.ewm-detail-wap').addClass('tj-hide')
            }, 100)
